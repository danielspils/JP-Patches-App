...
Current version: **0.8.4** (June 28, 2026). 25+ public releases since v0.1.0 on May 19. Public site live at **[jx-3p.com](https://jx-3p.com)** (built May 28–29). Release automation (`scripts/release.sh`) shipped May 29.
...
26. **Record-from-JX "didn't decode cleanly" — troubleshooting playbook.** When a capture fails to decode, get DATA before theorizing (the 2026-06-26 session burned two wrong theories first). 
    ...
    **Ruled OUT (2026-06-26 — don't re-chase):**
    - **Sample rate** — the on-disk WAV is always 44.1 kHz even when the KT device is at 48 kHz (Chromium resamples during capture). The "prefers 44.1 kHz" warning is real but is NOT the decode culprit; the codec sees clean 44.1 k. **⚠ Superseded by #27:** that 48→44.1 *real-time* resample DOES jitter the FSK cycle timing (~2×) enough to drop *dense* sequences — the rate itself isn't the culprit, the forced-44.1k *real-time resample* is. Fixed by capturing at the device's native rate.
    ...
    **FAILURE-MODE TRIAGE (2026-06-26 retrospective — these FOUR got conflated in one debugging marathon; capturePeak alone does NOT predict success).** Pull the temp WAV (`jp_*record_*.wav` in tmpdir), boost it, count total bit-0 cycles, run a sliding-window decode — and if it works on one machine but not another, compare `library.json` across them:
    - **~0 bit-0 cycles anywhere** → the capture contains no dump data: the JX dump didn't land in the recording window (capture timing / early auto-stop), the JX didn't dump that press, or a transient. **No software fix — a capture/hardware event, not the decoder.** (Seen 2026-06-26: the 14:48 capture had 1 bit-0 cycle total; cause never pinned down — do NOT assume a cable fault; Daniel pushed back on that.)
    - **Many bit-0 cycles, full buffer fails BUT a sub-window decodes** → the loud-idle **trim bug** → fixed by `findFskStartByFreq` (v0.8.5). The systemic, intermittent one (random success/fail at identical gain across weeks of `captureLog`).
    - **Many bit-0 cycles but even the best window only partially decodes** → a **marginal signal** (too-quiet capture whose boost amplified the noise floor into spurious cycles). Needs more level / cleaner cable, not code.
    - **Decodes on machine A but not machine B (same cable + JX + app version), or a Recalibrate loop** → an **over-hot saved calibration**. Compare `library.record.calibratedGain` + the `captureLog` gain/peak across the two machines (AirDrop the failing one's `library.json`; it lives at `~/Library/Application Support/jp-patches/library.json`). 2026-06-26: the Mini had a saved **14.23×** (peak 0.76–0.90 → all-null) while the laptop had **none** and used the 2.0× auto-default (peak 0.13 → success). Calibration aimed for peak ~0.78, which on a quiet-input machine needs ~14× — over-driving the capture; each failure re-prompted "Recalibrate" → re-landed hot → **loop**. **Fix: clear the device's saved gain → it reverts to the 2.0× auto-default** (the decode boost makes a hot capture unnecessary). v0.8.6 added the guardrails so this can't recur: **calibration target 0.78→0.45 + gain cap 30→12** ([app.js:8868] inline calc; mirror in `calibration-math.js`), and the recovery modal now shows **"Reset to auto-decode"** (clears the saved gain) instead of "Recalibrate" whenever a device has one.

    **DEAD ENDS from the marathon — do NOT re-chase (each disproven with data):** *sample rate (48 kHz)* — the on-disk WAV is always 44.1 k (Chromium resamples; constant since the original Record feature commit `9051d72`, worked for months); *clipping* — failing captures measured 0%; *browser DSP* (echo-cancel/noise-suppress/AGC) — the v0.8.4 `audioDiag` proved `processingActive:false` every time; *weak/absent bit-0* — a symptom of idle-in-the-trim, not capture quality; *auto-boost 0.7→0.92* — re-decoding a failing WAV at every boost 0.5–1.0 gave 0 patches, amplitude was never it; *bad/loose cable* — tempting (Daniel reseated once with an apparent fix) but it failed across two independent rigs and the real cause was software both times. **Reach for the data — temp WAV + `captureLog` + `library.json` — not the cable.** The only two real software causes: the loud-idle trim (v0.8.5) and the over-hot calibration (v0.8.6 guardrails). Related: #11 (boost), #12 (the now-secondary amplitude trim + its gain-scaled thresholds), #15 (dump duration).

27. **Tape-transfer record dropouts are JP's forced-44.1 kHz CAPTURE path (Chromium real-time-resampler jitter), NOT the analog gear — 2026-06-27.** *(An earlier same-day draft of this pitfall blamed the analog path; the QT-vs-JP test below disproved it. Keeping the correction visible — the wrong conclusion was very convincing and cost real time.)* The codec is lossless both directions; the loss is that **the WAV JP captures is timing-jittered**. JP forces `getUserMedia` + `AudioContext` to **44100 Hz** (`renderer/audio-capture.js:40,56,261`), so Chromium runs the KT cable's **native 48 kHz** input through its **real-time** resampler during capture, which roughly **doubles the FSK cycle-timing jitter** (long-cycle σ ≈ **1.00 vs 0.54**). At 44.1k the bit-0 short cycle is only ~5.5 samples, so **dense sequences** (gnarly's 1463-bit records) are maximally sensitive and lose ~3 of 8 pages; **patches** (286-bit) and sparse sequences tolerate it (∴ it "worked for months").
    - **PROOF (QT-vs-JP, same JX / cable / Mac):** a **single QuickTime capture** of one JX dump decodes **8/8** (verified on two individual dumps), and a **proper offline 48→44.1k resample** stays 8/8 at σ 0.56 — so *resampling per se is fine*. The **JP real-time capture** of the equivalent dump decodes **5/8** at σ **1.00**, with otherwise pristine crossings (0% anomalous, no silent gaps). The bug is the **real-time** resampler, not resampling. Also explains why **v0.6.5 fails identically** (same forced-44.1k capture path) and why the earlier analog/gear call was wrong.
    - **THE ROOT FIX: capture at the device's native rate** — drop the forced `44100` on the `AudioContext` + both `getUserMedia` constraint paths, and make rate-dependent code read `audioContext.sampleRate` (record-trim's `SHORT_MIN/MAX_SAMPLES` bit math; the WAV header already does; auto-stop budgets are ms-based ∴ rate-independent). The codec/decoder already handle 48k cleanly (QT proved 8/8). On a branch + Electron-validated before any release.
    - **Multi-dump merge** (prototyped 2026-06-27 — union checksum-valid pages across 2–3 dumps; Daniel's 4 real subsets {4,6,8}∪{6,8}∪{1,3,5}∪{3,6,7} merged to 7/8 byte-exact) remains a useful **robustness mitigation** but is NOT the root fix.
    - **TOOLING TRAP:** hand-rolled `_decode_patches` / `_decode_sequence_records` framing loops in a Python REPL give **FALSE results** — **always decode via the `jx3p` CLI (`wav-to-json` / `wav-to-seq-json`) as ground truth**; it disagreed with manual framing in both directions and was right both times. Related: #11, #12, #15, #17 (Chromium sample-rate caching), #26.
