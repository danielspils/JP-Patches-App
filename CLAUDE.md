...
**v0.8.5** (June 2026). 25+ releases since v0.1.0. Site live at [jx-3p.com](https://jx-3p.com); releases cut via `scripts/release.sh` (signed + notarized DMG, electron-updater auto-update). A Windows preview (CI-built NSIS, tagged `vX.Y.Z-win-preview`, prerelease) is kept in sync with every Mac release.
...
- **Windows port** 🚧 on the `windows-port` branch — **JX↔JP round-trip confirmed working on real hardware 2026-07-04** (Section 1 execFile refactor was the fix; the `main`-branch shell-string jx3p call is mangled by cmd.exe). Runs from source (`npm start`) against `C:\Users\<user>\JP-Patches`. **Requires Windows 10 (64-bit)+** — Win7/8/8.1 are a hard Electron floor (Electron dropped them in v23; we ship 35), and launch there aborts with a misleading *"is not a valid Win32 application"*. Open port items live in the task list (quiet-input gain auto-handling, `populatedPatches` counter bug, auto-zoom, first-Send venv-prewarm). Don't downgrade Electron to chase Win7.
...
32. **Windows input runs QUIETER than the Mac rigs** (~0.04 raw). At the 2× auto-decode default that's ~0.086 post-gain — UNDER the 0.10 signal-detect floor → "No audio detected" on a perfectly good dump. Fix is **Calibrate** (measure→boost, now offered on the no-signal recovery); the decode-time auto-boost (0.92) does the rest. Don't lower the Mac defaults to compensate — calibrate per device.
33. **Windows labels USB *outputs* "Speakers (…)" / "Headphones (…)"** → collides with the speaker-detection regex. Two casualties: (a) a saved USB cable got flagged "routed to speakers" (fixed — trust an explicitly-saved device); (b) the tape-dump-sound **MONITOR grabbed the cable itself and truncated JP→JX sends (bank D dropped)** → monitor is **gated OFF on win32**. Re-enabling needs a narrowed regex (match Realtek/High Definition Audio/Internal/Built-in, NOT generic Speakers/Headphones) + cable-exclusion by device GROUP, not exact id.
