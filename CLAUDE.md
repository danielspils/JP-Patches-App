...
26. **Record-from-JX "didn't decode cleanly" — troubleshooting playbook.** When a capture fails to decode, get DATA before theorizing (the 2026-06-26 session burned two wrong theories first). 
    ...
    **ROOT CAUSE — CONFIRMED 2026-06-26: a degraded physical cable contact, NOT the auto-calibration work.** Reseating the KT cable at both ends instantly restored a clean bit-0 cluster (9,655 short cycles → decode 7 records) at the **same ~0.17 `capturePeak`** that had produced all-null failures minutes earlier (bit-0 absent/fragmented). Same machine, same `gain: null` auto path, same code — only the connection changed. The "it regressed with auto-cal" correlation was coincidental timing. **➜ FIRST move on ANY new Record-from-JX decode failure: reseat the KT cable at both ends (JX tape jacks + Mac).** A dirty/loose contact rolls off high frequencies first, which kills the short bit-0 tone; it's directional, so Load-*to*-JX keeps working and masks it. The boost/threshold mechanism above is still a real *latent* fragility (a bigger boost on a quieter capture partly compensates for a weak bit-0 — that's why "lower gain seemed to help"), so hardening auto-cal to aim low / decode-verify remains worthwhile future work — but it was not the culprit here. Related: #11 (auto-boost), #12 (gain-scaled thresholds), #15 (sequence-dump duration), #17 (48 kHz advisory).
