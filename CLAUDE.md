...
Current version: **0.8.9** (August 15, 2026). Both Mac (signed/notarized DMG, electron-updater auto-update) and a synced Windows preview (`vX.Y.Z-win-preview`, CI-built NSIS) are live. 27+ releases since v0.1.0. Site at [jx-3p.com](https://jx-3p.com); releases cut via `scripts/release.sh`. **v0.8.7** shipped the two Windows-hardware-validated features (frequency-aware capture, trap #36; Windows app-audio routing, trap #33 / Task #21). **v0.8.8** added an anonymous, opt-out **usage ping** (first-launch notice + Settings toggle → relay `/ping`, once/day, version + country, no identifier) plus download + button-click geography tracking in the relay Worker (`/download/*`, GoatCounter mirror). The daily email (`scripts/download-report.mjs`) reports GitHub download counts and jx-3p.com button clicks as **two separate metrics** — clicks are never netted against or capped by downloads (there's no data linking a click to a completed download).
...
37. **Updater bumps take TWO releases to prove.** Every installed app self-updates using its OLD electron-updater: the first release cut after a bump is *fetched by the previous updater version*; the bumped code only RUNS starting the release after that. A smooth next rollout therefore proves nothing about the new updater. Context (Aug 2026, 6.3.9 → 6.8.9 for GHSA-p2f4-r6v6-j797): feed/release resolution under JP's win-preview + retro-attached-v0.8.9-assets scheme was verified against the changelog only, not exercised — watch the SECOND release after the bump for update failures.
...
33. **Windows labels USB *outputs* "Speakers (…)" / "Headphones (…)"** → collided with the speaker-detection regex. (a) a saved USB cable got flagged "routed to speakers" (fixed — trust an explicitly-saved device); (b) the tape-dump-sound **record MONITOR grabbed the cable itself and truncated sends** → was gated off on win32. **BOTH RESOLVED (Task #21, July 2026):** narrowed `WIN_SPEAKER_RE` + cable-exclusion by device GROUP (`selectSoundOutputDevice`), and — the robust fix — all app audio (button sounds, sequencer preview, record monitor) now shares ONE resolved sink that excludes the actual **capture-input device** by groupId, so it works for any interface + any speaker name. Preview also needed a startup `setPreviewSink` call (it only ran on device-change before → silent on a fresh launch). **Record monitor: re-gated OFF on Windows again** — routing is safe, but on a thrifty PC streaming the live capture out a speaker starves the main-thread ScriptProcessor and **drops random sequence pages** (verified: sounds ON dropped pages 2 then 6; sounds OFF clean ×3; jx3p CLI confirmed the pages were absent from the audio). It's a capture-integrity risk on slow machines until capture moves to **AudioWorklet** (off the main thread — the fix that brings the monitor back; see `docs/future-features.md`). The SEND monitor is unaffected (fixed WAV, no capture running).
