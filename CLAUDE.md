...
Current version: **0.8.4** (June 26, 2026). 25+ public releases since v0.1.0 on May 19. Public site live at **[jx-3p.com](https://jx-3p.com)** (built May 28–29). Release automation (`scripts/release.sh`) shipped May 29.
...
26. **Record-from-JX "didn't decode cleanly" — troubleshooting playbook.** When a capture fails to decode, get DATA before theorizing (the 2026-06-26 session burned two wrong theories first). 
    ...
    **ROOT CAUSE — a transient degraded *capture-device state*, cleared by re-enumerating the USB device (unplug/replug, or reboot). NOT a bad cable, NOT the auto-cal work.** Re-seating the KT USB cable restored a clean bit-0 cluster (9,655 short cycles → decode 7 records) at the **same ~0.17 `capturePeak`** that failed minutes earlier — same machine, same `gain: null` path, same code, **zero code changes**. The decisive clue (caught by Daniel): the SAME failure hit **both** of his setups — separate computers, JX-3Ps, AND KT cables — so two independent cable faults is implausible. A common failure points at the **shared layer** (the app's Chromium capture stack / macOS CoreAudio), not the hardware. Unplugging the USB connector and rebooting both work because both **re-enumerate the audio device and reset its cached/negotiated state**. Strongly consistent with **#17** (Chromium caches `getUserMedia` streams per deviceId — a stale or audio-**processed** cached stream, e.g. noise-suppression/AGC from a soft-constraint fallback, would strip exactly the high-freq bit-0 tone) and **#22** (USB off/on nudges CoreAudio to refresh). **➜ FIRST move on any new Record-from-JX decode failure: unplug/replug the KT USB cable (or reboot) to re-enumerate the device.** Directional, so Load-*to*-JX keeps working and masks it. **NOT yet pinned:** the exact bad state (cached stream with NS/AGC on, vs a CoreAudio format glitch) — the fix is to add capture-stream telemetry (`track.getSettings()` processing flags + the soft-constraint-fallback flag → `captureLog`) so the next failure is caught red-handed; a strong candidate code fix is to **force a fresh device stream on each capture** (defeat the Chromium per-deviceId cache) rather than rely on the user re-plugging. The boost/threshold fragility above is real but was not the trigger. (Earlier 2026-06-26 note wrongly called this a physical cable contact — corrected after Daniel pointed out it failed across two independent rigs.) Related: #11 (auto-boost), #12 (gain-scaled thresholds), #15 (sequence-dump duration), #17 (48 kHz advisory), #22 (USB/CoreAudio reset).
