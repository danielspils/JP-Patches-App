...
25. **A WAV that decodes to nothing must be REJECTED, never applied or re-routed in a loop.** Tones⇄Sequence auto-reroute is one-shot (`planImportReroute`); every apply-to-active-banks path runs the all-default/identical guard before snapshotting. Gate new import entry points with `describeUnsupportedImport` (names MP3/MP4/etc.). *(Junk = anything whose FSK timing is gone — lossy re-encode, a baked-in metronome click, etc.)*
