...
(function () {
  ...
  // How long the signal gate must stay OFF (no FSK data) before the end-of-dump
  // auto-stop fires. MUST exceed the JX's inter-bank DIVIDER. A tone dump is:
  // [pilot][bank C data][pilot][bank D data]. That mid-dump "divider" is a
  // second ~5 s PILOT TONE — 4096 bits of continuous binary-1 symbols at ~882 Hz,
  // regenerated by the synth's P8051 microcontroller (source: phaysis.com JX-3P
  // tape-dump decoder; corroborated by Daniel's ~4.5 s hardware measurement).
  // 4096 bits ÷ 882 Hz ≈ 4.64 s, and since it's a fixed firmware bit-count it's
  // the SAME on every JX-3P (only the dump LEVEL varies per unit). The FSK gate
  // counts only the SHORT (3675 Hz, binary-0) cycles, so an all-1s pilot reads
  // as no-FSK → the gate correctly goes OFF across the divider. At 1000 ms — and
  // even 3500 ms — the auto-stop fired DURING that pilot and truncated to bank C
  // only (16 patches; bank D all-default, one repeated name). The live gate's
  // OFF span runs ~0.5 s longer than the pilot (0.5 s detection window fill), so
  // ~4.64 s reads as ~5.1 s. 7000 ms clears it with ~2 s margin while still
  // ending promptly on the real post-dump silence; a longer window only adds
  // trailing wait — dump-timeout / safety-timeout remain the runaway backstops.
