...
- ~~**Auto-converging decode — eliminate the Record-from-JX calibration step.**~~ ✅ **SHIPPED v0.8.2** (June 14, 2026) — but NOT via the sweep this item proposed. The decode-time gain/limiter *sweep* was built, tested, and **reverted**: the plain peak-boost (`AUTO_BOOST_TARGET` 0.7→0.92, shipped v0.8.1) decodes every real capture on its own, and the limiter rungs over-clip a clean FSK tone (whose tone IS its own peak — no dead transient to clip away). What shipped instead: a never-calibrated device captures at `DEFAULT_CAPTURE_GAIN` (2.0×) and the boost finds the level — no mandatory calibration. Two-pass calibration survives as a failure fallback; a clipping capture auto-steps the gain down. See `renderer/record-flow.js` + CLAUDE.md "Calibration Celebration" theme. **Lesson for future me: don't rebuild the sweep.**
