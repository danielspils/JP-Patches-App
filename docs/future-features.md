...
- **In-app share + explore workflow (Daniel, 2026-06-10).** Daniel's envisioned user-facing shape for Phases 1+2 — refines (doesn't replace) the sketches above:
  ...
  - **Modal design v1 (Daniel mockup, 2026-06-10).** Single modal per sub-tab combining browse + share (replaces the separate share-hover-button idea):
    ...
    - **Unsettled:** (1) instant-publish vs curated-queue semantics — decides whether the third state reads "shared" or "submitted", and whether unshare needs an identity story (a local secret share-token per submission allows self-serve unshare without accounts); (2) sharer name + notes entry — likely a small confirm step on share-click, which also hosts the undo toast.
