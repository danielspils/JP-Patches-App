...
- **In-app share + explore workflow (Daniel, 2026-06-10).** Daniel's envisioned user-facing shape for Phases 1+2 — refines (doesn't replace) the sketches above:
  ...
  - **Explore**: a button at the bottom of each Library sub-tab opening a modal with the **3 latest** community tones / sequences (was 10; Daniel narrowed it 2026-06-10), each one-click portable into the user's local library. A "explore more" link deep-links to the jx-3p.com community tabs. The small cap is deliberate: in-app search/browse UX gets tricky beyond a few entries, so browse-at-scale lives on the site. (The latest-N list is just the first N entries of `index.json` sorted by `addedAt` — no extra endpoint needed.)
  ...
  - **Terminology (Daniel, 2026-06-10): borrow / lend.** The community library is "the user lending library." Users **borrow** files (Roland-red buttons) and **lend** their own (Roland-blue buttons — formerly "share"). In-app explore buttons read "explore the user lending library" on both sub-tabs; site pages + issue templates use lending language throughout (template FILENAMES keep the share-* slugs — they are URL-referenced).
  - **SHIPPED (2026-06-10): explore modal with borrow + lend v1.** Borrow fetches the live manifest (3 latest); lend ships on the GITHUB-ISSUE transport — consent checkboxes gate blue lend buttons over the user's own items, the confirm step (name/hometown/notes, prefs-persisted) copies the payload JSON to the clipboard and opens the pre-filled issue form via openExternal. RELAY SHIPPED same day (relay/ in repo — Cloudflare Worker, deploy steps in relay/README.md): lend POSTs to lend.jx-3p.com → Worker files the GitHub issue (no user GitHub account needed); 'submitted' state persists on the item (item.lending = {token, submittedAt, issueUrl}); the clipboard + GitHub-form flow remains as automatic fallback whenever the relay is unreachable. Still open: self-serve withdraw via the stored token (worker endpoint not yet built), borrowed-item exclusion from the lend list (no community marker yet).
  - **Modal design v1 (Daniel mockup, 2026-06-10).** Single modal per sub-tab combining browse + lend:
    - Top: "Recent User Shared Tones" — list of the 3 latest community files, each with a Roland-red **borrow** button (one-click import into local library, with community marker + re-download conflict prompt).
    - Middle: "Share Your Tones" — two consent checkboxes ("I am sharing my own tones (no one else's)", "anybody can download and use these tones") gate the Roland-blue **lend** buttons. Checkboxes reset on every modal open — deliberately NOT persisted (Daniel, 2026-06-10: the re-check is a good reminder of what lending means). Packages carrying a community marker are excluded from the shareable list — structurally can't re-share someone else's files.
    - Lend button states: lend (blue) → submitted (activated blue tone) → click again to withdraw. Implement the change-your-mind window as an undo toast ("Lent — undo, 30s"), not a literal delayed send.
    - **Lend-click confirm step (decided 2026-06-10):** clicking lend opens a small confirm modal with three fields, then the actual Lend commit. This is also where the undo toast lives after commit. One extra click, but it's what makes the community catalog readable. Fields + example placeholder copy (Daniel's):
...
- **Auto-converging decode — eliminate the Record-from-JX calibration step.** Calibration exists to set a per-device software gain, but gain is a software multiply applied *before* decode, so it can be searched automatically on an already-captured buffer — no re-dump needed (the expensive part is the manual JX Save + 6–30 s transmission; re-decoding the same buffer is sub-second). Proposed design: capture once at a conservative gain that won't clip any line input; then sweep the decode normalization over that buffer until the checksums validate, returning the first level that decodes. The only constraint that genuinely needs human help is **clipping at capture** (unrecoverable data loss) — detect it and tell the user to turn down their interface; everything else (too quiet, wrong level) is fixable post-capture. **The hard part:** plain peak-normalization (today's auto-boost, raised to 0.92 in v0.8.1) won't fully replicate the "crank into the red" rescue, because a loud transient (pilot onset) eats the headroom and leaves the data quiet — so the sweep needs a *limiter* step (clip the dead peak, push the body up), ideally inside the codec gated on checksum success. Cable-vs-interface variance (50–100× input range) is NOT a blocker — it only sets the capture starting level, which the sweep absorbs. Discussed with Daniel 2026-06-12; v0.8.1 shipped the AUTO_BOOST_TARGET bump as the interim relief. Estimate: a real pass with test recordings — medium, mostly in the fork's codec.

- **Delete-confirm guard for lent items.** Deleting a library item that has a `lending` record also deletes its secret withdraw token — the only self-serve way to pull the published entry from the lending library. (Discovered 2026-06-11: Daniel lent David Nixon's sequence, borrowed it back, deleted the original — the catalog entry is now withdrawable only via `scripts/remove-lend.sh`.) Fix: when the trash confirm fires on an item with `item.lending`, add a line — "This also removes your ability to withdraw it from the lending library." ~15 minutes.
