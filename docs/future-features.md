...
- **In-app share + explore workflow (Daniel, 2026-06-10).** Daniel's envisioned user-facing shape for Phases 1+2 — refines (doesn't replace) the sketches above:
  ...
  - **SHIPPED (2026-06-10): explore modal with borrow + lend v1.** Borrow fetches the live manifest (3 latest); lend ships on the GITHUB-ISSUE transport — consent checkboxes gate blue lend buttons over the user's own items, the confirm step (name/hometown/notes, prefs-persisted) copies the payload JSON to the clipboard and opens the pre-filled issue form via openExternal. No relay yet, so: 'request opened' state is session-only (no submitted-tracking, no withdraw), and borrowed items aren't excluded from the lend list (no community marker yet). The relay upgrade swaps the mechanics (one-click submit, token withdraw) without changing the UI.
