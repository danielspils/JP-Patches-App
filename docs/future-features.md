...
- **AudioWorklet migration.** `ScriptProcessor` (in `setupAudioGraph`, `audio-capture.js`) is deprecated; Chromium will remove it eventually. `AudioWorklet` is the replacement (lower latency, own thread). ~half day, mostly mechanical — but it processes in 128-sample blocks, so the `captured.push(...)` accumulation changes shape. **Now higher-priority:** because ScriptProcessor runs on the main thread, main-thread load during capture (the re-enabled JX→JP record monitor) drops audio samples on a thrifty PC and corrupts random sequence pages (2026-07-05 — the monitor is re-gated OFF on Windows because of this). Moving capture to AudioWorklet makes it immune to that load and **is the fix that re-enables the Windows record monitor** (and hardens capture generally on slow machines).
...
- **Sequence editor doesn't refresh on a new-sequence import** (Daniel, 2026-07-05; Mac + PC). After transferring a sequence to the Library (JX→JP) or dropping a WAV with a new sequence, the sequence **editor keeps showing the *previous* sequence** — the newly-imported one doesn't display until you click to another sequence and back. Behaves like a stale-render/caching issue: the import adds the sequence to the library but doesn't re-point/re-render the editor's active view. Likely fix: on a successful sequence import, set the active-sequence selection to the new entry and force the editor re-render (the click-away-and-back path already does this — call the same refresh on import). ~30–60 min once located.

- **Record progress timeline out of sync** (Daniel, 2026-07-05). The Record-from-JX progress bar / segment indicator runs but drifts from the actual dump. Likely a consequence of Fix 2 (frequency-aware live detection): `firstSignalMs` now fires when bit-0 FSK is *detected* (~0.5 s after data starts, once the rolling window fills) rather than on the amplitude threshold, and the end-of-dump silence window grew to 7 s — so the progress-pacing math (tuned to the old amplitude timing + inter-bank behavior) no longer lines up, especially across the C→D pilot. Re-tune the timeline against the new gate timings (`firstSignalMs`, `activeMs`, `END_OF_DUMP_SILENCE_MS`). Deferred until the Windows capture/audio work is fully wrapped.

- **8-page sections on the sequence capture timeline** (Daniel, 2026-07-05). The Record timeline's single green "Sequence" section could be split into **8 sub-sections** (one per page) that light as each page transfers — matching the gate-anchored tone timeline (Bank C / Divider / Bank D). Needs a **separator detector**: pages are delimited by 48-bit inter-record separators (~54 ms of all-1s), invisible to the main 0.5 s frequency gate. They ARE distinguishable — sequence data never has >11 consecutive 1-bits (the format's `INTERTONE_RUN` rule) while a separator is 48 in a row — so a short-window "long-1-run" counter could find them and count records → pages (8 pages × 2 redundant copies = 16 records). Moderate signal-processing + its own hardware tuning; do NOT fake it with equal-time division (that reintroduces the estimate drift the gate-anchored timeline removed).
