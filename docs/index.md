...
What I need now are computer literate Mac users to download JP Patches from GitHub for testing. I'm not a programmer or IT support person so this would require a person(s) who can navigate the world of GitHub. Specifically, this is an "unapproved" app that a user must permission onto their computer. Until I get an Apple Developers license, it will remain a little tricky to download. Installing involves bypassing macOS's Gatekeeper warning — sometimes just a right-click → Open, sometimes a one-line command in Terminal on newer macOS. [Download JP Patches for macOS](https://github.com/danielspils/JP-Patches-App/releases/latest)
...
[Download JP Patches for macOS](https://github.com/danielspils/JP-Patches-App/releases/latest)
